...
class cl_buttons_panel_01:
    def __init__(self, master):
        ...
        labelLine = tk.Label(frame, text="Rotate around line AB")
        labelA = tk.Label(frame, text="A: ")
        entryA = tk.Entry(frame, textvariable = A, width=10)
        labelB = tk.Label(frame, text="B: ")
        entryB = tk.Entry(frame, textvariable = B, width=10)
        labelRotate = tk.Label(frame, text="Rotate by: ")
        entryDegree = tk.Entry(frame, textvariable = degrees, width=5, justify="right")
        labelDegrees = tk.Label(frame, text="°")
        
        self.rotateButton = tk.Button(frame, text="Rotate", fg="blue")#, command=self.button2_callback)
        
        labelLine.pack(side=tk.LEFT)
        labelA.pack(side=tk.LEFT)
        entryA.pack(side=tk.LEFT)
        labelB.pack(side=tk.LEFT)
        entryB.pack(side=tk.LEFT)
        labelRotate.pack(side=tk.LEFT)
        entryDegree.pack(side=tk.LEFT)
        labelDegrees.pack(side=tk.LEFT)
